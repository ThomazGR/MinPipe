...
class CheckLibs:
    ...
    def __check_kallisto(self) -> None:
        ...
        if all(contains in str(kall.stdout) for contains in ["kallisto", "/"]) and Path(str(kall.stdout)).is_dir():
    ...
    def __check_trim_galore(self) -> None:
        ...
        if all(contains in str(tg.stdout) for contains in ["trim_galore", "/"]) and Path(str(tg.stdout)).is_dir():
    ...
    def __check_fastqc(self) -> None:
        ...
        if all(contains in str(fqc.stdout) for contains in ["fastqc", "/"]) and Path(str(fqc.stdout)).is_dir():
