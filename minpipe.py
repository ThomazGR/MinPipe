...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run full pipeline for every samples given."
    )
    parser.add_argument(
        "-s",
        "--samples",
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=False,
        help="<Optional> Complementary for paired-ended",
        type=str,
    )
    parser.add_argument(
        "-i",
    ...
    parser.add_argument(
        ...
        nargs="?",
        required=False,
        help="<Optional> File format to be used in the \
            end of file names.",
    )
    parser.add_argument(
        "--threads",
    ...
    if args.json is not None and args.yaml is not None:
        print(
            "Both YAML and Json detected. Please pass only 1 file through argument handling."
        )
        exit()
