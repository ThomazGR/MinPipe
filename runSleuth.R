...
#BiocManager::install(c("biomaRt", "rhdf5", "ggplot2","dplyr","clusterProfiler","org.Hs.eg.db","org.Mm.eg.db"))
...
create.args <- function() {
    ...
    Usage:
      runSleuth.R (-f --file) [-g --groups] [--no-volcano] [-o --organism] [-p --path] (-r --results)
...
if (!dir.exists(arg$results)) {
    ...
    print(paste(
            system("ls | grep results", intern = TRUE, show.output.on.console = FALSE)
    ), collapse = " | ")
    quit()
}

if (!dir.exists(arg$path) & !arg$no_volcano) {
...
if (length(strsplit(arg$groups, ",")[[1]]) <= 1) {
    stop("-g/--groups argument has only 1 value (group) which is not right.")
}
